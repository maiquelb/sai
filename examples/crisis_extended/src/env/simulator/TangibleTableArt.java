...
public class TangibleTableArt extends cartago.tools.GUIArtifact {
	...
	private static final ImageIcon iconMail = new ImageIcon(TangibleTableArt.class.getResource("/simulator/images/mail.png"));
	...
	public void setup() {
		...
		linkActionEventToOp(frame.to3Button,"message_button_click");
	...
	@INTERNAL_OPERATION void message_button_click(ActionEvent ev){
		putMessageTangibleDialog(new MessageTangibleObject());
	}
	...
	private void putMessageTangibleDialog(MessageTangibleObject tangible){
		JPanel p = new JPanel();
		JPanel labels = new JPanel(new GridLayout(0,1,2,2));
		labels.add(new JLabel("Row:", SwingConstants.RIGHT));
		labels.add(new JLabel("Column:", SwingConstants.RIGHT));		
		...
		JTextField txtY = new JTextField(5);
		controls.add(txtY);
		labels.add(new JLabel("To:", SwingConstants.RIGHT));
		labels.add(new JLabel("Message:", SwingConstants.RIGHT));
		JTextField txtTo = new JTextField(10);
		JTextField txtMessage = new JTextField(30);
		controls.add(txtTo);
		controls.add(txtMessage);
		p.add(controls, BorderLayout.CENTER);
		int option = JOptionPane.showConfirmDialog(null, p, "Inform the coordinates", JOptionPane.OK_CANCEL_OPTION);
		if(option==JOptionPane.OK_OPTION){
			tangible.setFrom("xxx");
			tangible.setTo(txtTo.getText());
			tangible.setMessage(txtMessage.getText());
			execInternalOp("internal_putMessageTangible", tangible, Integer.parseInt(txtX.getText())-1, Integer.parseInt(txtY.getText())-1, frame.cbUsers.getSelectedItem().toString());
		}
	...
	@INTERNAL_OPERATION void internal_putMessageTangible(TangibleObject tangibleObject, int x, int y, String actor){
		defineObsProperty("tangibleIn", tangibleObject.getId(),x+1,y+1);
		this.cells[x][y].addTangibleObject(tangibleObject);
		//The coordinates "x" and "y" are added in 1 as, to the external world (external to the table applications)
		//the coordinates start in "1" insted of starting in "0" as in the internal representation		
		signal("putTangible",this.tableId, tangibleObject.getId(),x+1,y+1,actor,((MessageTangibleObject)tangibleObject).getTo(),((MessageTangibleObject)tangibleObject).getMessage(),actor);		
	}
	...
	@INTERNAL_OPERATION void internalCheckin(ActionEvent ev){
		...
		this.frame.to3Button.setEnabled(true);
		this.frame.to1Button.setVisible(true);
		this.frame.to2Button.setVisible(true);
		this.frame.to3Button.setVisible(true);				
	...
	class MyFrame extends JFrame {    
		...
		public MyFrame(JLabel[][] cells, String tableId){
			...
			to3Button = getButton("Message", iconMail);
			...
			to3Button.setVisible(false);


			header2.setVisible(false);
			header2.add(new JLabel("Tangible objects:"));
			header2.add(to1Button);
			header2.add(to2Button);
			header2.add(to3Button);
