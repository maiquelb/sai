...
+!have_a_house 
   <- !setup_sai;
      ...
      !execute.  // (simulate) the execution of the construction
...
+!setup_sai
   <- !setup_sai_wsp_ora4mas; //each plan "setup_sai_X" sets links the workspace X to the SAI engine    
      ...
      ?jcm__ws("wsp_sai",WspSai); //look to the SAI workspace
   	  ...
   	  //set up norms for scheme
      lookupArtifact("bhsch",Npl2Sai); //look to the artifact that encloses the norms corresponding to the scheme   
      focus(Npl2Sai);       
      ...
      lookupArtifact("hsh_group",Group);//look to the artifact that encloses the norms corresponding to the group   
      focus(Group);       
